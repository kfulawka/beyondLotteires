...
etPrepare <- function(x) {
  ...
  t_sub <- function(x, n_t = 65) {
    ...
    for ( i in 1:n_t) {
      ...
      try(si[i] <- which(x$CPNum == f_start[i]), silent = T)
      
      if ( is.na(si[i]) ) {
        si[i] <- grep(paste('^', i-1, '_onset', sep = ''), x$CPNum)
        f_start[i] <- x$CPNum[grepl(paste('^', i-1, '_onset', sep = ''), x$CPNum)]
      }
      ...
      if (i == 1) {
        ...
        x_dec$CPNum[which(x_dec$CPNum == f_start[i]):which(x_dec$CPNum == t_choice[i])] <- i-1
